...
namespace Sc.Util.System
{
					...
					IXmlSerializable
	{
		...
		/// <returns>This object for chaining.</returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public LongFlags Invert(int position)
		{
		...
		/// <returns>This object for chaining.</returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public LongFlags Invert(params int[] positions)
		{
			ulong flags = Flags;
			try {
				foreach (int position in positions) {
		...
		/// <summary>
		/// Returns the upper 32 bits as a UInt.
		/// </summary>
		/// <returns>The upper 32 bits, unsigned.</returns>
		public uint Get32HighBits()
			=> (uint)(Flags >> 32);
		...
		public override string ToString()
			=> $"{nameof(LongFlags)}"
					+ "["
					+ $"{Convert.ToString((byte)(Flags >> 56), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 48), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 40), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 32), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 24), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 16), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)(Flags >> 8), 2).PadLeft(8, '0')}"
					+ $" {Convert.ToString((byte)Flags, 2).PadLeft(8, '0')}"
					+ "]";
	}
}
